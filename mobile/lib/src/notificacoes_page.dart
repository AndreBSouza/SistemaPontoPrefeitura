...
/// Tela "Notificações" — lista de notificações do servidor.
...
class _NotificacoesPageState extends State<NotificacoesPage> {
  ...
  Future<void> _carregar() async {
    ...
    try {
      final lista =
          await widget.api.listarNotificacoes(widget.servidorId);
      if (mounted) setState(() => _itens = lista);
    } catch (e) {
      if (mounted) setState(() => _erro = mensagemDeErro(e, padrao: 'Não foi possível carregar as notificações.'));
    }
  }
                            ...
                            return ListTile(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                    item.enviadaEm.length >= 10
                                        ? item.enviadaEm.substring(0, 10)
                                        : item.enviadaEm,
